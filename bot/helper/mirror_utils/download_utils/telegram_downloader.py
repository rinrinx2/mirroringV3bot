    ...
    def add_download(self, message, path, filename):
        ...
        if media is not None:
            ...
            if download:
                ...
                LOGGER.info(f'Mengunduh file telegram dengan id: {media.file_id}')
                threading.Thread(target=self.__download, args=(_message, path)).start()
            else:
                self.__onDownloadError('File sudah diunduh!')
        else:
            self.__onDownloadError('Tidak ada dokumen dalam pesan balasan')

    def cancel_download(self):
        LOGGER.info(f'Membatalkan unduhan atas permintaan pengguna: {self.gid}')
