...
class GoogleDriveHelper:
    ...
    def _on_upload_progress(self):
        if self.status is not None:
            ...
            LOGGER.debug(f'Mengupload {self.name}, chunk size: {get_readable_file_size(chunk_size)}')
    ...
    def __upload_empty_file(self, path, file_name, mime_type, parent_id=None):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Mengupload dengan Odier Mirror Bot',
            'mimeType': mime_type,
        }
    ...
    def deletefile(self, link: str):
        ...
        try:
            ...
            if "Berkas tidak ditemukan" in str(err):
                ...
                msg = "Ada yang salah, periksa log"
        finally:
            return msg
    ...
    def switchServiceAccount(self):
        ...
        LOGGER.info(f"Mengalihkan ke {SERVICE_ACCOUNT_INDEX}.json service account")
        self.__service = self.authorize()
        return True
    ...
    def upload(self, file_name: str):
        ...
        LOGGER.info("Mengupload File: " + file_path)
        ...
        if os.path.isfile(file_path):
            try:
                ...
                LOGGER.info("Mengupload ke G-Drive: " + file_path)
            ...
            try:
                ...
                if result is None:
                    raise Exception('Unggahan telah dibatalkan secara manual!')
                LOGGER.info("Mengupload ke G-Drive: " + file_name)
        ...
        LOGGER.info("Menghapus file/folder yang diunduh..")
        return link
    ...
    def cloneFolder(self, name, local_path, folder_id, parent_id):
        LOGGER.info(f"Menyinkronkan: {local_path}")
    ...
    def edit_telegraph(self):
        ...
        for content in self.telegraph_content :
            ...
            Telegraph(access_token=telegraph_token).edit_page(path = self.path[prev_page],
                                 title = 'Odier mirror bot Search',
                                 author_name='Odier Mirror Bot',
                                 author_url='https://t.me/OdierBambi',
        ...
        if response["files"]:
            ...
            for file in response.get('files', []):
                if file.get('mimeType') == "application/vnd.google-apps.folder":  # Detect Whether Current Entity is a Folder or File.
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        sfurl = requests.get(f'https://{SHORTENER}/api?api={SHORTENER_API}&url={furl}&format=text').text
                        msg += f"<b><a href={sfurl}>🕊️Drive Link</a></b>"
                    else:
                        msg += f"<b><a href={furl}>🕊️Drive Link</a></b>"
                    if INDEX_URL is not None:
                        url_path = requests.utils.quote(f'{file.get("name")}')
                        url = f'{INDEX_URL}/{url_path}'
                        urls = f'{INDEX_URL}/{url_path}?a=view'
    ...
    def cancel_download(self):
        ...
        self.__listener.onDownloadError('memmbatalkan Download !')
