...
def clean_download(path: str):
    if os.path.exists(path):
        LOGGER.info(f"Membersihkan download: {path}")
        shutil.rmtree(path)
...
def exit_clean_up(signal, frame):
    try:
        LOGGER.info("Harap tunggu, sementara kami membersihkan unduhan dan menghentikan unduhan")
        clean_all()
        sys.exit(0)
    except KeyboardInterrupt:
        LOGGER.warning("Paksa Keluar sebelum pembersihan selesai!")
        sys.exit(1)
